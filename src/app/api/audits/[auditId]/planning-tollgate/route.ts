...
import { getPlanningTollgateViewModel } from "@/lib/planning-tollgate/view-model";
import { createSupabaseAdminClient } from "@/lib/supabase/admin";

const updatePlanningTollgateDraftSchema = z.object({
  markdown: z.string().min(1),
});

type PlanningTollgateDocumentRow = {
  id: string;
...
export async function GET(_request: Request, context: { params: Promise<{ auditId: string }> }) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq("audit_id", auditId)
      .eq("document_type", "PLANNING_TOLLGATE")
      .order("updated_at", { ascending: false })
      .limit(1)
      .maybeSingle<PlanningTollgateDocumentRow>();
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Unable to load the planning tollgate.",
      },
      { status: 400 },
    );
  }
...
export async function POST(_request: Request, context: { params: Promise<{ auditId: string }> }) {
  try {
    const { auditId } = await context.params;
    const viewModel = await getPlanningTollgateViewModel(auditId);
    const sanitizedMarkdown = sanitizeDraftMarkdown(viewModel.renderedTemplate);
    const preview = buildNarrativePreview(sanitizedMarkdown);
    const sourceRecordKey = `planning-tollgate-${auditId}`;
    ...
    return NextResponse.json({
      draft: {
        ...
        templateName: persistedDraft?.template_name ?? "System Planning Tollgate Template",
        title: persistedDraft?.title ?? "Planning Tollgate Draft",
      },
    });
  } catch (error) {
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Unable to generate the planning tollgate.",
      },
      { status: 400 },
    );
  }
...
export async function PATCH(request: Request, context: { params: Promise<{ auditId: string }> }) {
  try {
    const { auditId } = await context.params;
    const body = updatePlanningTollgateDraftSchema.parse(await request.json());
    ...
    if (!existingDraft) {
      return NextResponse.json({ error: "No planning tollgate draft exists for this audit yet." }, { status: 404 });
    }

    if (isPlanningDraftLockedForReview(readString(existingDraft.source_payload, "review_status"))) {
      return NextResponse.json({ error: "This planning tollgate is locked while it is in review." }, { status: 409 });
    }
    ...
    return NextResponse.json({
      draft: {
        ...
        templateName: data?.template_name ?? "System Planning Tollgate Template",
        title: data?.title ?? "Planning Tollgate Draft",
      },
    });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: error.issues[0]?.message ?? "Invalid planning tollgate payload." }, { status: 400 });
    }

    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Unable to update the planning tollgate.",
      },
      { status: 400 },
    );
  }
...
export async function DELETE(_request: Request, context: { params: Promise<{ auditId: string }> }) {
  try {
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Unable to reset the planning tollgate.",
      },
      { status: 400 },
    );
  }
...
function resolveDraftTimestamp(document: PlanningTollgateDocumentRow | null | undefined) {
  if (!document) {
    return null;
  }

  return (
    readString(document.source_payload, "edited_at") ??
    readString(document.source_payload, "generated_at") ??
    document.updated_at
  );
}
