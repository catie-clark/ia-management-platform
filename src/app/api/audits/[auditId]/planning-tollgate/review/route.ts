...
export async function PATCH(request: Request, context: { params: Promise<{ auditId: string }> }) {
  try {
    ...
    const document = result.data ?? (await loadPlanningArtifactDocument(auditId, "PLANNING_TOLLGATE"));
    ...
    await safelyCreateNotification(async () => {
      if (body.action === "submit") {
        await createNotificationsForRole({
          auditId,
          detail: `${document?.title ?? "Planning tollgate"} is waiting for manager review.`,
          entityId: document?.id ?? null,
          entityType: "planning_artifact",
          eventType: "PLANNING_TOLLGATE_MANAGER_REVIEW",
          linkHref: `/planning?mode=live&auditId=${auditId}`,
          role: "MANAGER",
          title: "A planning tollgate is ready for manager review",
          tone: "warning",
        });
        return;
      }
      ...
      if (body.action === "send_back") {
        if (document?.owner_user_id) {
          await createNotificationForUserId({
            auditId,
            detail: body.comment?.trim() || `${document?.title ?? "Planning tollgate"} was sent back for updates.`,
            entityId: document.id,
            entityType: "planning_artifact",
            eventType: "PLANNING_TOLLGATE_SENT_BACK",
            linkHref: `/planning?mode=live&auditId=${auditId}`,
            title: "A planning tollgate was sent back to you",
            tone: "warning",
      ...
      if (nextReviewStatus === "DIRECTOR_REVIEW") {
        await createNotificationsForRole({
          auditId,
          detail: `${document?.title ?? "Planning tollgate"} is waiting for director review.`,
          entityId: document?.id ?? null,
          entityType: "planning_artifact",
          eventType: "PLANNING_TOLLGATE_DIRECTOR_REVIEW",
          linkHref: `/planning?mode=live&auditId=${auditId}`,
          role: "DIRECTOR",
          title: "A planning tollgate is ready for director review",
          tone: "warning",
        });
        return;
      }
      ...
      if (nextReviewStatus === "APPROVED" && document?.owner_user_id) {
        await createNotificationForUserId({
          auditId,
          detail: `${document?.title ?? "Planning tollgate"} completed the review workflow.`,
          entityId: document.id,
          entityType: "planning_artifact",
          eventType: "PLANNING_TOLLGATE_APPROVED",
          linkHref: `/planning?mode=live&auditId=${auditId}`,
          title: "Your planning tollgate was approved",
          tone: "success",
          userId: document.owner_user_id,
        });
      }
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Unable to update the planning tollgate review workflow.",
      },
      { status: 400 },
    );
  }
...
async function safelyCreateNotification(callback: () => Promise<void>) {
  try {
    ...
    console.error("Unable to create planning tollgate review notification", error);
  }
}
