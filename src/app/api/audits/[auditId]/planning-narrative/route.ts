  ...
  try {
    ...
    return NextResponse.json({
      draft: {
        ...
        templateName: persistedDraft?.template_name ?? "System Planning Narrative Template",
        title: persistedDraft?.title ?? "Planning Narrative Draft",
      },
    });
  } catch (error) {
    return NextResponse.json(
...
export async function PATCH(request: Request, context: { params: Promise<{ auditId: string }> }) {
  try {
    ...
    if (isPlanningDraftLockedForReview(readString(existingDraft.source_payload, "review_status"))) {
      return NextResponse.json({ error: "This planning narrative is locked while it is in review." }, { status: 409 });
    }
...
function resolveDraftTimestamp(document: PlanningNarrativeDocumentRow | null | undefined) {
  if (!document) {
    return null;
  }
